...
이 실습의 user 계정은 admin 권한이 없기 때문에, 내용을 읽어보시고 실습은 다음 파트인 Persistent Volume & Persistent Volume Claim에서 계속합니다.
...
`hostPath` 전략을 사용하되 이를 볼륨으로 직접 구성하지 않고 영구 볼륨(PV) 및 영구 볼륨 클레임(PVC)을 사용하겠습니다. 
...
위 내용을 확인해보면, 이제 '볼륨' 에 대한 정보는 _영구 볼륨_ 개체에 있으며, 더 이상 Pod에 작성할 필요가 없습니다. 

아래 명령어를 통해 영구 볼륨(PV)을 생성합니다.
...
볼륨이 설정되면 Terminal#3 을 업데이트하여 볼륨의 새 위치인 `{mount-dir}` 을 살펴보겠습니다.
...
kbd:[CTRL+c]를 눌러 기존 watch 명령을 종료합니다.

그리고 새로운 watch 명령을 실행합니다.
