...
~~~~
Kubernetes에서 컨테이너의 휘발성 문제 해결하기
이제 우리가 살펴볼 문제는 컨테이너가 휘발성(volatile)이라는 점입니다.
즉, 컨테이너를 중지하면, 그 파일 시스템에 기록된 모든 것이 사라진다는 것이죠.

어떤 경우에는 이게 괜찮을 수도 있습니다.
하지만 만약 데이터베이스를 Pod 내부에 배포하려고 한다면?
모든 데이터를 잃고 싶지는 않겠죠.
데이터를 **영구적으로 저장(persist)**하고,
Kubernetes 클러스터의 모든 노드에서 접근 가능해야 합니다.

왜냐하면, 특정 노드에서 Pod를 실행하고 있다가
이를 스케일링하면 Pod가 다른 노드로 이동할 수 있기 때문입니다.
그리고 이동한 노드에서도 동일한 데이터를 사용할 수 있어야 합니다.

이런 이유로 Kubernetes에서는 볼륨(Volumes) 개념을 제공합니다.
아마 Kubernetes 1부에서 이미 볼륨 개념을 배우셨을 것입니다.
오늘은 이를 실제로 어떻게 사용하는지 살펴보겠습니다.
