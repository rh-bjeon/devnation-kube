...
kubectl 커맨드는 Kubernetes에 기본 제공되는, Kubernetes API에 대한 Thin Wrapper입니다. 
OpenShift oc 커맨드는 kubectl 커맨드의 상위 집합으로, kubectl 커맨드뿐만 아니라 추가적으로 OpenShift 기능에 대한 커맨드를 사용할 수 있습니다.
