...
Pod를 직접 생성하는 대신 Deployment를 생성하면, Deployment는 ReplicaSet 
원하는 수의 Pod가 항상 사용 가능하도록 ReplicaSet을 생성하고 Pod를 대체하기 위한 전략(예: RollingUpdate)을 지정하는 
Deployment는 일부 명시적 시나리오를 제외하고는 Pod를 직접 생성하는 것보다 바람직합니다 
